...
format:
    revealjs:
        ...
        include-after-body:
            ...
            - bd/js/bootswatch-theme-demo.html
        embed-resources: false
execute:
    echo: true
---
![](inst/img/linux-distro.jpeg){fig-align="center" height="100%"}
...
![](inst/img/worst-dashboard.jpg){fig-align="center" height="100%"}
...
And I have made some not so great looking work.
...
And, unfortunately, the researchers didn't discover a comprehensive checklist that will make every person trust everything you produce.
...
::: {.fragment}
![](https://datawrapper.de/cdn-cgi/image/quality=85,width=778,f=auto,fit=cover/https://kirby.datawrapper.de/media/pages/blog/colors-for-data-vis-style-guides/072a079d76-1740123150/200801_styleguide-nyt-2.png){.absolute left=0px bottom=70px height="80%"}

![](inst/img/font.png){.absolute right=0px bottom=110px height="70%"}
:::
...
::: {.fragment}
![](inst/img/color-categories.png){.absolute left=50px}
![](inst/img/color-patterns.png){.absolute right=100px}
:::
::: {.fragment}
![](inst/img/color-emphasis.png){.absolute bottom=-50px height="50%"}
:::
...
Color can help people understand. It can distinguish categories, reveal patterns, and direct attention to important information
:::

## Color Shapes Attention and Tone

![](inst/img/color-effects.png){.absolute top=200px}

::: notes
But color also shapes the overall experience. A palette can feel calm or exciting, serious or trustworthy.
...
![](inst/img/map-narrative.png){.absolute}
![](inst/img/map-tooltips.png){.absolute right=300px}
![](inst/img/chart-axes.png){.absolute bottom=0px left=300px}
![](inst/img/chart-annos.png){.absolute bottom=0px right=0px}
...
![](inst/img/font-size.png){.absolute height="82%"}
...
::: notes
Differences in size, weight, and style help people recognize what they should read first, what comes next, and what is supporting detail.

It also shapes the overall voice of your product by contributing to the mood, and tone we communicate before anyone reads a word.

Which one of these notes do you want to find waiting for you? Same words, different personality.

Of course, before a font can give our work personality or make it feel like ours, it has to be readable.

It needs to work at the sizes we actually use with clear letters, numbers, and enough contrast against the background.
:::

---

![](inst/img/papyrus.gif){.absolute left=0px bottom=0px height="70%"}
![](inst/img/papyrus-meme.jpg){.absolute right=0px height="60%"}
...
![](inst/img/trustworthy-palette.png){.absolute top=200px height="70%"}
![](inst/img/map-narrative.png){.absolute top=200px right=0px height="70%"}

::: notes
Color helps direct attention and set the tone while typography creates hierarchy and gives your work a voice.

Neither changes the underlying information.

But together they create the first impression long before someone evaluates the data.
:::
...
I need fonts, button styles, and a brand color palette with approximately 47 coordinated shades of gray.
...
::: notes
But this foundation also means our work can look a lot like every other dashboard or report we've ever seen.
...
![](inst/img/bootswatch-logo.png){.absolute right=500px height="90%"}

::: notes
That is where Bootswatch comes in.

Bootswatch gives us a collection of ready-made themes for Bootstrap that can be easily applied to our HTML based outputs.
...
theme_images <- purrr::imap_chr(themes, \(theme, i) {
  ...
  sprintf(
    '![](https://bootswatch.com/%s/thumbnail.png){.absolute top="calc(%s%% + 70px)" left="%s%%" height="%s%%"}',
    theme,
    top,
    left,
    img_height_pct
...
::: notes
Bootswatch gives us a range of coordinated starting points.

The structure and content stay the same, but we're able to quickly change the look and feel of our data products without having to write custom CSS.

Bootswatch might not give us our final design instantly, but with one simple change we can get a lot closer to the experience we want.
:::

## Applying Bootswatch Themes {.btd-slide}

::: {.gd-stage-wrap}
<div id="btd-stage"></div>
:::
[]{.fragment .gd-frag id="btd-vapor"}
[]{.fragment .gd-frag id="btd-minty"}
[]{.fragment .gd-frag id="btd-sketchy"}
...
We choose a preset Bootswatch theme, pass it to the app through the theme argument, and it changes the styling across the entire application.

One line of code changes the entire visual experience without redesigning the document or touching its content.
:::
...
## A Preset Is a Starting Point {.center}

::: notes
A Bootswatch preset gives us a coordinated starting point.

But it doesn't know our organization’s colors, our preferred fonts, or how we want our products to feel.
...
![](https://datawrapper.de/cdn-cgi/image/quality=85,width=2582,f=auto,fit=cover/https://kirby.datawrapper.de/media/pages/blog/colors-for-data-vis-style-guides/a4eaf9456a-1740123150/Artboard-6.png){.absolute left=300px height="90%"}

::: notes
Reusing these choices does more than save us time.

Over time it helps work from the same source feel recognizable
...
That familiarity matters because people often use what they recognize as one part of deciding whether something feels credible and trustworthy.
...
::: notes
Typography can also create a strong connection.

An organization might use different colors from one publication to the next, but a repeated typeface can still hold the work together.

The New York Times is a useful example. Their visualizations can use very different palettes, but the recurring title font Cheltenham helps them remain recognizable.

Differing styles from the same font create variety and hierarchy across titles, labels, and supporting text.

So brand recognition does not require every product to look exactly the same, it just needs enough shared identity to make separate products sound like they are speaking with the same voice.

And when we define these brand elements in one reusable place, we don't have to recreate that identity one product at a time.
:::
...
Color and typography shape the first impression, but consistency is what makes that impression recognizable over time, and I can define them once in a single file.

The first section establishes my color palette and gives each color a purpose. I am not just saving hex codes. I am identifying which color should be used for text, backgrounds, primary elements, warnings, errors, and other interface states.

The typography section works the same way. It defines the fonts, weights, sizing, and spacing for both body text and headings.

Once those decisions live in brand.yml, the brand is no longer dependent on me remembering every design decision. Those decisions are recorded in a format the tools can use.
:::
...
::: notes
Now in Quarto, I keep the Bootswatch preset as the foundation and add the brand file.
...
I spend less time rebuilding the same styling decisions and more time working on the content.
:::
...
At our foundation, we have Bootstrap. It gives our product structure and functioning components.

Apply a Bootswatch theme for a more polished starting point without any design effort.

On top of that, brand.yml stores the choices that make the product recognizable: our colors, fonts, and visual identity.

And then there is still room to polish any components you want to design yourself. The final details that are specific to this product and this audience.

Styling does not have to be one huge task left looming at the end of your projects.
...
::: {.fragment}
![](inst/img/linux-distro.jpeg){.absolute height="90%"}
:::

::: notes
Before I finish, let’s go back to the website from the beginning.

A few of you may have recognized it. This is DistroWatch, a widely used and trusted website that provides information about available Linux distributions for over 25 years.
...
DistroWatch didn't become trustworthy because I explained what it was.
...
Most of us can't put up a questionable-looking dashboard and say, “don't worry. People already know we're credible.”
...
![](inst/img/talk-page-qr.png){.absolute bottom=100px height="50%"}

::: {.absolute bottom="0px"}
{{< fa link >}} [shelbylevel.org/talks/first-impressions-matter](https://shelbylevel.org/talks/first-impressions-matter)
:::

::: {.absolute right="0px" bottom=100px}
{{< fa brands github >}} [github.com/shelbylevel](https://github.com/shelbylevel)

{{< fa brands linkedin >}} [linkedin.com/in/shelbylevel](https://linkedin.com/in/shelbylevel)

{{< fa envelope >}} [slevel20@gmail.com](mailto:slevel20@gmail.com)
:::
:::
