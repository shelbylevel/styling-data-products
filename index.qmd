...
theme_images <- purrr::imap_chr(themes, \(theme, i) {
  ...
  glue::glue(
    '![](https://bootswatch.com/{theme}/thumbnail.png){{.absolute top="{top}%" left="{left}%" height="{img_height_pct}%"}}'
  )
})
...
But repeated colors, typography, and visual patterns can make them feel like they belong to the same family.

Over time, people don't have to relearn the visual language with every new product.

They can recognize it and spend more time engaging with the information.
:::
